...
### Option 1: 모빌리티 도메인 가치 극대화 (권장, 1~2주)
```
Week 1:
...
**현재 평가의 한계:**
지금까지 배치 실험/스크린샷 등에서 OD 를 대부분 **지하철역 좌표** 로 잡아 평가해왔다
(강남역 ↔ 홍대입구역 등). 하지만 실사용 OD 는 **"집/오피스/공원/카페 거리"** 처럼
역에서 수백 m ~ 1 km 떨어진 위치끼리 연결된다. 역→역 구간은 지하철이 거의 항상
이기므로, 현재 평가 방식으로는 **MaaS 복합 경로의 가치가 정량적으로 드러나지 않는다**.

**개선 방향:**
- "현실 OD 세트" 정의: 아파트 단지 / 오피스 / 공원 / 상권 / 카페거리 등
  역에서 떨어진 좌표 30~50쌍 확보
- 각 OD 를 `TRANSIT_ONLY` vs `SPECIFIC+PERSONAL_EBIKE` 등으로 실측 비교
- 지표: 평균/p95 시간 단축, mixed 추천 채택률, 이동수단별 퍼스트/라스트마일 비율
- 결과를 `docs/performance/real-user-benchmark.md` 로 누적 기록

**실측 힌트 (2026-04-22 스팟체크):**
- 서초 아파트(37.4850,127.0320) → 성수 카페거리(37.5420,127.0554)
  * `TRANSIT_WITH_BIKE` 28분 vs `TRANSIT_ONLY` 36분 → **8분 단축**
- 방배 아파트 → 성수 카페거리 → **8분 단축**
- 대부분의 "역↔역" OD 에서는 mixed 경로가 1등 되지 않음 (평가 편향 증거)

#### 파일
- `docs/performance/real-user-benchmark.md` (신규)
- `scripts/benchmark/real-od-batch.py` (신규, 자동 배치)

#### 공수
1~2일

#### 설계 포인트
> "역 좌표 편향" 을 스스로 발견하고, 실사용자 OD 로 평가 기준을 재정의했습니다.
> MaaS 복합 경로의 가치가 실제로 드러나는 OD 세트를 구축해 엔진 튜닝의 근거로 활용합니다.
...
## C. 모빌리티 도메인 특화 ⭐ EV/자동차 제조사 플랫폼 연계
...
#### 기록
> "EV 운전자를 위한 경로를 고도화했습니다. 목적지 인근 충전소의 재고/혼잡도를 실시간 반영해, 긴 이동 시 경유 충전이 최적인지 목적지 충전이 최적인지 자동 판단합니다. 향후 EV 플랫폼 차량 데이터(예: E-GMP 등)와 연동하면 남은 배터리 기반 실시간 충전 경로 재탐색까지 가능합니다."
