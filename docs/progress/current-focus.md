...
- 라스트마일에서는 최근접 대여소 힌트가 있는 허브를 우선하도록 보조 정렬을 적용해 `NO_PICKUP`를 줄이는 방향으로 튜닝 중이다
- 다만 2026-03-19 재검증 기준으로 `NO_PICKUP` 비중 자체는 크게 줄지 않았고, 다음 단계는 허브 후보 생성 자체에 대여소 접근성을 더 직접 반영하는 것이다
- 이후 허브 후보 생성 단계에 최근접 대여소 접근성 기반 필터를 추가했고, 최신 재검증에서는 `NO_PICKUP`가 `6 -> 4`로 감소하면서 `no-mixed` 샘플 중 1건은 mixed 대안을 갖게 됐다
- 2026-03-19 추가 튜닝:
  ...
  - 추가로 퍼스트마일 패턴(B/D)에서는 출발지 `pickup preflight`를 먼저 확인해, pickup이 전혀 없는 경우 허브 조합 시도 자체를 줄이도록 보강
