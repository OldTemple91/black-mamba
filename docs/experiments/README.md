...
- `od-samples.no-mixed.json` 4건에서는 최신 pruning 기준으로 `NO_CANDIDATE_HUB`, `NO_PICKUP`, `NO_DROPOFF` 진단이 실제로 수집됨
- 같은 `no-mixed` 세트에서 `recommendedGenerationPhaseCounts`는 `FIRST_MILE 4`, `LAST_MILE 4`로 집계되어 어느 구간에서 병목이 큰지 바로 확인 가능
- 최신 코드(`mobility_segment` 캐시 포함) 기준 `no-mixed` 재실행에서는 `SAME_PICKUP_DROPOFF`가 사라지고 `NO_CANDIDATE_HUB 6`, `NO_PICKUP 1`, `NO_DROPOFF 1`로 재분류됨
