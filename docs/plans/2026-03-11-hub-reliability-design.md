...
2. `HubSearchPort`로 후보 검색 책임을 분리
3. `Location`에 대응되는 `Hub`를 함께 생성
4. 전략 클래스는 `Hub`를 우선 사용하되, 실제 경로 조립은 기존 로직 재사용
5. UI에는 후보점 대신 `Hub` 이름/타입 노출
...
이 점이 자동차 제조사·카셰어 사업자/SDV/PBV 관련 포지션에서 가장 중요한 차별화 포인트가 된다.
