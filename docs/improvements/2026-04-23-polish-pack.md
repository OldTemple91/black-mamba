...
Tier 1(관측성 4축) + Tier 2(Carbon/Weather) 까지 끝낸 상태에서 **포트폴리오 완성도** 를 올리는 3종 세트를 한 번에 정리.

| 구멍 | 왜 메꿔야 하는가 |
|------|---------------|
| 알고리즘 설계 결정이 ADR 에 없음 | 발표관이 "왜 A*/Dijkstra 를 안 썼나" 물어봤을 때 문서 근거가 없음 |
...
**발표 질문 직격**: "왜 표준 알고리즘 안 썼어요?" → ADR-007 을 보여준다.
...
운영 배포 시엔 `GF_AUTH_ANONYMOUS_ENABLED=false` 로 완전 차단 권장. 현 설정은 포트폴리오/데모 편의 우선.
...
> "Tier 1·2 로 관측성 4축과 MaaS 정체성을 갖춘 후, **포트폴리오 완성도를 결정짓는 세 구멍** 을 한 번에 메웠습니다.
>
> 하나 — **ADR-007** 로 'A\*/Dijkstra 를 왜 안 썼나' 를 공식 문서로 답변. 대안 4개를 비교한 Decision Record 는 '생각한 걸 남기는 엔지니어' 라는 시그널입니다.
>
> 둘 — **M-4/5/6 운영 마감** 으로 Loki 노이즈 / Grafana 권한 / Tempo·Loki 재기동 데이터 소실을 한 번에 정리. 평소 간과되는 디테일이 발표에서 '이 사람 운영 안 해본 게 아니구나' 를 증명합니다.
>
