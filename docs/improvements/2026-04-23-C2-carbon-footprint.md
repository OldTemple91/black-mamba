...
### 4-2. EV 차량 비교는 미포함
현재 "자가용 171 g/km" 은 휘발유 기준. EV 비교는 한국 전력 믹스 기준 ~60 g/km 인데, **별도 파라미터화 필요** (EV/자동차 제조사 어필용).
...
>
> 설계의 핵심은 **'경로 탐색 로직에 침투하지 않는' 후처리**입니다. Route 가 만들어진 후 `withCarbon()` 으로 첨부되므로 탐색 알고리즘엔 0 영향. 다음 단계로 EV 차량 비교 계수 (한전 전력 믹스 60 g/km) 를 추가하면 EV 제조사 플랫폼 사용자에게 더 맞춤화된 비교가 가능합니다."
