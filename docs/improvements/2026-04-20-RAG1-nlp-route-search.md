...
### 공고 매칭
(자동차 제조사) (메모) 담당 업무/우대사항에 다음이 명시됨:
- **RAG 파이프라인 설계 및 최적화**
- **AI 모델 통합 및 성능 튜닝**
- **벡터 DB, Embedding 기반 검색 이해 및 활용**
- **대화형 AI/RAG 서비스 구조 설계 및 운영 역량**
...
- 공고 우대사항 **"벡터 DB, Embedding 기반 검색"** 은 Phase 2에서 Qdrant/pgvector 도입 예정
- 경로 이력 임베딩 + 유사 경로 추천
...
### 공고 매칭 언급
> "(자동차 제조사) (메모의) **RAG 파이프라인 설계** 항목을 이 프로젝트로 직접 구현했습니다. Phase 2에서는 **Qdrant 벡터 DB**에 경로 이력을 임베딩으로 저장하고, **코사인 유사도 기반 유사 경로 추천**으로 확장할 계획입니다."
